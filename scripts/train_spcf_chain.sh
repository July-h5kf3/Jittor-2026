...
MASTER=train_logs/spcf_chain_${TS}.log
mkdir -p train_logs
echo "[chain] start $(date)" | tee -a "$MASTER"

# fresh start
rm -rf experiments/spcf_vm experiments/spcf_cvm experiments/spcf_spcf
...
run_stage () {
  ...
  NP=2 bash scripts/train_ddp.sh "configs/task/${task}" > "train_logs/stage_${name}_${TS}.log" 2>&1
  pkill -9 -f 'run.py --task' 2>/dev/null; pkill -9 -f mpirun 2>/dev/null; sleep 3
...
run_stage train_spcf_vm.yaml   vm   experiments/spcf_vm
run_stage train_spcf_cvm.yaml  cvm  experiments/spcf_cvm
run_stage train_spcf.yaml      spcf experiments/spcf_spcf

echo "[chain] === PREDICT + EVAL on localtest === $(date)" | tee -a "$MASTER"
pkill -9 -f 'run.py --task' 2>/dev/null; sleep 2
rm -rf results_local
python run.py --task configs/task/predict_spcf_local.yaml > "train_logs/predict_spcf_local_${TS}.log" 2>&1
