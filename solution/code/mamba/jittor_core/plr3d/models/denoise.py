...
from ..patch_merge import merge_patch_predictions
...
class DenoiseNet(nn.Module):
    ...
    def patch_based_denoise(
        ...
        output_numpy, fallback_indices = merge_patch_predictions(
            np.ascontiguousarray(denoised_patches.numpy(), dtype=np.float32),
            np.ascontiguousarray(positions.numpy(), dtype=np.int32),
            np.ascontiguousarray(all_distances.numpy(), dtype=np.float32),
            np.ascontiguousarray(cloud[0].numpy(), dtype=np.float32),
            strategy=merge_strategy,
            temperature=softmax_temperature,
        )
        output = jt.array(output_numpy).float32()
    ...
    def architecture_metadata(self) -> Dict[str, object]:
        return {
            ...
            "default_patch_merge": "softmax010",
