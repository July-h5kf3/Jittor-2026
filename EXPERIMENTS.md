...
`1.05` 取 1.03/1.06 平台区间的中点作为提交候选。收益小于 0.1，必须通过线上结果确认，不能据此宣称超过 CVM-002。
...
1. 用已准备的 alpha 1.05 配置生成一次线上提交，确认 local2 的 +0.05 是否能迁移。
2. 实现法向/表面感知 endpoint loss，直接对齐 P2S，同时约束 CD 覆盖。
3. 实现坐标图 + 特征图双图解码器，减少薄面跨表面误连。
4. 若需要补历史路线，再复训 CVM-006；不再优先投入 LDC/time-stage adapter。
