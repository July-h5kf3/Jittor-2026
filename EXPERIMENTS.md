...
- PNX 仍提供了有效误差多样性。固定输出空间集成 `0.70*ROT + 0.30*PNX` 的 CD/P2S/总分为 `57.45936157 / 90.20204048 / 73.83070103`，相对 ROT 为 `+0.15605581 / -0.00112173 / +0.07746704`；总分 95% CI `[+0.03541857,+0.12107780]`，`44/62` 云获胜，LOCO 范围 `[+0.05797595,+0.08707134]`。PNX 权重 0.20 与 0.40 也分别提高 `+0.07190266`、`+0.06480754`，说明收益位于宽平台而非尖锐单点。
- 200 云候选流水线于 `2026-07-20T04:50:15+08:00` 正常结束：PNX raw、mean/var、pass2、CV two-pass 与最终 70/30 ensemble 均为 200/200；最终数组全部为 `float32 (50000,3)`、finite，且逐云点数匹配 noisy。ZIP `submission_results/result_rot001_pnx001_twopass_ens_p030_20260720.zip` 大小 `110770733` 字节，SHA256 `ebcec509073ac86078a08639441e6c6ec1ed6c713b92575b04bf4b1a149b3279`，已通过独立 archive validator。该包只作为待批准候选，未自动提交。
...
## UNI-001：旋转不变的局部间距保持损失（已实现，待正式训练）

- 假设：当前主要瓶颈是 CD 覆盖率；手工 tangent repulsion 能提高 CD，却以近等量 P2S 回撤为代价。与其在推理后手工推开点，训练期直接匹配 clean 采样的局部邻距分布，可能让网络学会减少收缩，同时由原 pointwise endpoint 监督继续约束表面贴合。
- 唯一训练变量：在 CVM 与 SPCF 最终 endpoint 上，以 clean-kNN 图构造 `L_spacing = Huber(log(d_pred/d_clean))`。固定从按半径排序的 1000 点 patch 中等距选 256 点，`k=8`；只比较边长，不比较边方向，因此对平移和旋转不变，也避免重复已失败的 edge-vector 过约束。权重 `lam_spacing=0.50`，现有所有配置默认 `0`。
- 风险控制：该损失不增加任何参数，旧 checkpoint 的 state key 与推理函数完全不变。CPU 测试覆盖 exact-zero、收缩/膨胀双向惩罚、control 默认关闭、配置隔离和 inference bitwise equivalence，5 项均通过。
- 权重标定：在 ROT checkpoint 的合成曲面诊断中，单位权重对 CVM 主损失贡献 `0.215617`（主损失 `2.040590`，10.57%），对 SPCF 主损失贡献 `0.259148`（主损失 `5.666746`，4.57%）；使用 `0.50` 后预计约占 5.28% 与 2.29%，属于温和辅助而非主导目标。
- V100 per-rank `B=8,N=1000` 的 CVM/SPCF forward-backward 均通过，allocator 分别约 `5.61/5.56 GiB`，远低于 16GB。训练链固定为 `train_spcfgfnuni001_cvm.yaml -> train_spcfgfnuni001.yaml`，保持 ROT 的初始化、真实旋转、噪声、四卡 batch、seed 123 和优化器不变。
- 判定：必须完成 canonical local2 raw/mean-var/two-pass、相对 ROT 的配对 CI、类别与 LOCO 后再保留；validation loss 仅用于 checkpoint 选择，不作为胜出证据。
