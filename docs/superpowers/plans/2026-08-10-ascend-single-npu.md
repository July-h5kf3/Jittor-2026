...
python3 -m unittest tests.test_source_policy tests.test_backend tests.test_ascend_environment_contract tests.test_device_wiring tests.test_acl_primitives
...
- Fresh corrected local gate after the source-audit fix `95ac2d535fd4c145cfc83232844fee52770ecb56`: `python3 -m compileall -q .` exited 0; the exact unittest command above, including `tests.test_acl_primitives`, ran 61 tests with 55 passed and 6 clean skips because the new `.git` audit regression adds one passing source-policy test to the historical 60-test gate. It includes source policy, backend, environment-contract, device-wiring, Task 7 smoke-schema checks, and all six locally skipped ACL primitive cases. `python3 tools/audit_source_archive.py --root .` exited 0 with `audit_pass=true`, 80 files, and 2,990,428 source bytes; `.git` is excluded identically whether it is a linked-worktree pointer file or a normal checkout directory. `git diff --check origin/new...HEAD` exited 0.
...
- Historical post-documentation snapshot: after the evidence commit `8abacc80030e1777d222f37fa69380a7e4088a23`, its ordinary push exited 0 and a second `scripts/sync_ascend.sh` exited 0. At that snapshot, the local worktree was clean with `HEAD=origin/new=8abacc80030e1777d222f37fa69380a7e4088a23`; the local and remote sync manifests both contained 80 files with SHA-256 `fe3a4fe39307d4fce557fd6048d904c6f780c86065348ae2c58a54a4360f13c4`. This is distinct from the earlier `a0878c6` synchronization and is not the current repository state.
- Later historical snapshot: after clarification commit `baee94e25408588bbb4121a5ce1caef1fd5ef396`, its ordinary push and the following `scripts/sync_ascend.sh` both exited 0. At that snapshot, `HEAD=origin/new=baee94e25408588bbb4121a5ce1caef1fd5ef396`, the worktree was clean, and both 80-file sync manifests had SHA-256 `3b45d765515e6619f4f314ffb74ee2f79bce8cfa2014714be1af77ccb1d8da9b`.
...
- Repository evidence can only name a preceding synchronization that was already completed. The current evidence correction commit will be ordinarily pushed and synchronized after it is created; its final `HEAD`/`origin/new` equality and local/remote manifest are therefore supplied by the fresh handoff verification, avoiding a fabricated self-SHA or an infinite documentation loop.
